...
function GetIban() {
    ...
    if (!iban) { // إذا كان الحقل فارغًا
        result.innerHTML = "لم يتم إدخال أي إيبان";
        return;
    }
    ...
    if (!ChechIban(iban)) {
        ...
        result.innerHTML = "الإيبان صحيح";
        result.className = "text-success";
    
        const samaCode = iban.substring(4, 6); // استخراج رمز SAMA
        const bank = banks.find(b => b.samaCode === samaCode); // العثور على البنك بناءً على رمز SAMA
    
        const bn = document.getElementById('bank'); // اسم البنك
        const bankResult = document.getElementById('bank1'); // رمز SAMA
    
        if (bank) {
            bn.innerHTML = "اسم البنك:";
            bankResult.innerHTML = `${bank.nameAr} (رمز SAMA: ${samaCode})`;
        } else {
            bn.innerHTML = "اسم البنك:";
            bankResult.innerHTML = `غير معروف (رمز SAMA: ${samaCode})`;
        }
}
}
